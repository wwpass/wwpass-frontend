...
describe('WWPassCryptoPromise', () => {
  test('Create WWPassCryptoPromise', () => {
    return WWPassCryptoPromise.getWWPassCrypto('mockTicket');
  });

  test('Encrypt/decrypt string', () => {
    return WWPassCryptoPromise.getWWPassCrypto('mockTicket').then(
        crypto =>
            crypto.encryptString(testString)
  ...
  test('Encrypt/decrypt arrayBuffer', () => {
    return WWPassCryptoPromise.getWWPassCrypto('mockTicket').then(
        crypto =>
            crypto.encryptArrayBuffer(testBuffer)
                .then(encrypedBuffer => crypto.decryptArrayBuffer(encrypedBuffer))
                .then(
                    decryptedBuffer =>
                        expect(equalAb(testBuffer,decryptedBuffer)).toBeTruthy()));
  });

  test('Invalid ticket', () => {
    return WWPassCryptoPromise.getWWPassCrypto('invalidTicket').then(()=>{expect(true).toBe(false);}, (error) => {
...
describe('WWPassCryptoPromise with AES-CBC', () => {
  test('Create WWPassCryptoPromise', () => {
    return WWPassCryptoPromise.getWWPassCrypto('mockTicket','AES-CBC');
  });

  test('Encrypt/decrypt string', () => {
    return WWPassCryptoPromise.getWWPassCrypto('mockTicket','AES-CBC').then(
        crypto =>
            crypto.encryptString(testString)
                .then(encrypedString => crypto.decryptString(encrypedString))
                .then(
...
describe('WWPassCrypto', () => {
  ...
  test('Create WWPassCrypto', () => {
    return (new WWPassCrypto('mockTicket')).cryptoPromise;
  });
...
const testBuffer = new Uint8Array([
  89,  117, 224, 105, 0,   104, 144, 97,  64,  225, 134, 219, 226, 209, 110,
  21,  16,  249, 38,  90,  114, 31,  186, 57,  175, 30,  185, 190, 239, 181,
  177, 134, 126, 145, 243, 209, 90,  175, 182, 9,   33,  155, 232, 92,  19,
  221, 244, 43,  56,  198, 243, 128, 51,  8,   136, 32,  14,  124, 102, 39,
  245, 66,  138, 119, 174, 83,  254, 227, 233, 223, 16,  233, 24,  118, 145,
  59,  142, 152, 247, 73,  42,  214, 168, 219, 167, 218, 12,  1,   87,  35,
  92,  154, 143, 107, 244, 203, 45,  38,  29,  56,  18,  243, 204, 26,  163,
  160, 26,  75,  69,  5,   34,  235, 246, 50,  2,   177, 47,  187, 232, 138,
  135, 192, 131, 77,  178, 148, 13,  238, 193, 119, 100, 140, 15,  244, 213,
  18,  161, 10,  157, 197, 71,  46,  38,  122, 30,  7,   122, 172, 19,  198,
  113, 203, 241, 122, 4,   28,  30,  26,  131, 214, 163, 177, 157, 49,  150,
  9,   155, 29,  152, 152, 127, 229, 255, 65,  127, 188, 147, 40,  228, 129,
  153, 211, 115, 231, 217, 248, 62,  13,  141, 107, 182, 207, 224, 144, 217,
  78,  86,  97,  211, 243, 33,  159, 67,  110, 121, 124, 2,   151, 106, 30,
  18,  33,  202, 81,  128, 82,  156, 157, 234, 25,  157, 127, 210, 162, 60,
  96,  145, 127, 146, 187, 149, 175, 107, 4,   91
]).buffer;

// From https://www.ltg.ed.ac.uk/~richard/unicode-sample.html
const testString=`
Basic Latin
